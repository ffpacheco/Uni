...
class Graph {
  ...
  public:
    ...
    bool dfs2(int v, std::vector<bool> &keeper) {
        nodes[v].visited = true;
        keeper[v]        = true;
        for (auto e : nodes[v].adj) {
            int w = e.dest;
            if (!nodes[w].visited) {
                if (dfs2(w, keeper)) return true;
            } else if (keeper[w]) return true;
        }
        keeper[v] = false;
        return false;
    }
    ...
    bool hasCycle() {
        std::vector<bool> keeper(n, false);
        for (int i = 0; i < n; i++) nodes[i].visited = false;

        for (int i = 0; i < n; i++) {
            if (nodes[i].visited == false) {
                if (dfs2(i, keeper)) return true;
            }
        }
        return false;
    }
