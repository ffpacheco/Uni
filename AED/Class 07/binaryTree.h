...
template <class T> class BTree {
  ...
  public:
    ...
    void sumOne(int c, Node *n, std::vector<int> &sums) {
        if (n == nullptr) return;
        if (c == 0) sums[c] += n->value;

        else {
            sumOne(c - 1, n->left, sums);
            sumOne(c - 1, n->right, sums);
        }
    }

    std::vector<int> sumLevels() {
        std::vector<int> sums(height() + 1, 0);
        for (int i = 0; i < height() + 1; i++) { sumOne(i, root, sums); }
        return sums;
    }
};
