...
class Graph {
  ...
  public:
    ...
    void dfs(int v, std::vector<bool> &visited, std::stack<int> &finishStack) {
        ...
        for (auto e : nodes[v].adj) {
            int w = e.dest;
            if (!visited[w]) { dfs(w, visited, finishStack); }
        }
        finishStack.push(v);
    }
    ...
    Graph transpose() {
        Graph t(n, true);
        for (int u = 1; u <= n; ++u) {
            for (auto e : nodes[u].adj) { t.addEdge(e.dest, u, e.weight); }
        }
        return t;
    }
    ...
    int countSCCs() {
        ...
        for (int i = 1; i <= n; ++i) {
            if (!visited[i]) { dfs(i, visited, finishStack); }
        }
